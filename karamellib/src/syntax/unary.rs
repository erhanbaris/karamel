...
impl UnaryParser {
    fn parse_suffix_unary(parser: &SyntaxParser) -> AstResult {
        ...
        if let Some(token) = parser.peek_token() {
            if token.token_type.is_symbol() {
                ...
                if let Some(operator) = parser.match_operator(&[KaramelOperatorType::Increment, KaramelOperatorType::Deccrement]) {
    ...
    fn parse_prefix_unary(parser: &SyntaxParser) -> AstResult {
        ...
        if let Some(operator) = parser.match_operator(&[KaramelOperatorType::Addition, KaramelOperatorType::Subtraction, KaramelOperatorType::Increment, KaramelOperatorType::Deccrement, KaramelOperatorType::Not]) {
            parser.cleanup_whitespaces();
            ...
            match operator {
                ...
                KaramelOperatorType::Addition | KaramelOperatorType::Subtraction => {
                    let opt = match operator {
                        KaramelOperatorType::Addition => 1_f64,
                        KaramelOperatorType::Subtraction => -1_f64,
                        _ => 1_f64,
                    };
