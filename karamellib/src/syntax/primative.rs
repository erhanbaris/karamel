...
impl PrimativeParser {
    pub fn parse_basic_primatives(parser: &SyntaxParser) -> AstResult {
        ...
        let result = match &token.unwrap().token_type {
            ...
            KaramelTokenType::Keyword(keyword) => match keyword {
    ...
    pub fn parse_list(parser: &SyntaxParser) -> AstResult {
        let index_backup = parser.get_index();
        if parser
            .match_operator(&[KaramelOperatorType::SquareBracketStart])
            .is_some()
        {
            ...
            if parser
                .match_operator(&[KaramelOperatorType::SquareBracketEnd])
                .is_none()
            {
    ...
    pub fn parse_dict(parser: &SyntaxParser) -> AstResult {
        let index_backup = parser.get_index();
        if parser
            .match_operator(&[KaramelOperatorType::CurveBracketStart])
            .is_some()
        {
            ...
            loop {
                ...
                let key = match key_ast {
                    Ok(KaramelAstType::Primative(primative)) => match &*primative {
                ...
                dict_items.push(Rc::new(KaramelDictItem { key, value: Rc::new(value.unwrap()) }));
            ...
            if parser
                .match_operator(&[KaramelOperatorType::CurveBracketEnd])
                .is_none()
            {
...
impl SyntaxParserTrait for PrimativeParser {
    fn parse(parser: &SyntaxParser) -> AstResult {
        map_parser(parser, &[Self::parse_dict, Self::parse_list, Self::parse_parenthesis, Self::parse_module_path, Self::parse_symbol, Self::parse_basic_primatives])
    }
}
