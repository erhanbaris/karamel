...
impl Tokinizer<'_> {
    ...
    pub fn add_token(&mut self, start: u32, token_type: KaramelTokenType) {
        let token = Token { line: self.line, start, end: self.column, token_type };
...
impl KaramelTokenType {
    pub fn is_symbol(&self) -> bool {
        matches!(self, KaramelTokenType::Symbol(_))
    }

    #[allow(dead_code)]
    pub fn is_keyword(&self) -> bool {
        matches!(self, KaramelTokenType::Keyword(_))
    }
