...
#[derive(Clone, Debug, PartialEq, Error, EnumIter, EnumMessage, EnumDiscriminants)]
pub enum KaramelErrorType {
    ...
    #[strum(message = "134")]
    FileReadError { filename: String, error: String },
    ...
    #[strum(message = "139")]
    FunctionArgumentNotMatching { function: String, expected: u8, found: u8 },

    #[error("'{function}' fonksiyonu sadece {expected} parametresini kabul ediyor")]
    #[strum(message = "140")]
    FunctionExpectedThatParameterType { function: String, expected: String },

    #[error("Doğrulama başarısız (Sol: {left:?}, sağ: {right:?})")]
    #[strum(message = "141")]
    AssertFailedWithArgument { left: Rc<KaramelPrimative>, right: Rc<KaramelPrimative> },
    ...
    #[strum(message = "146")]
    ModuleParseError { name: String, error: String },
...
impl From<KaramelErrorType> for KaramelError {
    fn from(item: KaramelErrorType) -> Self {
        KaramelError { column: 0, line: 0, error_type: item }
    }
}
...
pub fn generate_error_message<T: AsRef<str>, E: Borrow<KaramelError>>(data: T, error: E) -> String {
    ...
    let lines = data.as_ref().split('\n').collect::<Vec<_>>();
    let line = lines.get(error_ref.line as usize).unwrap();
    format!(
        "...\r\n{}\r\n{:>width$} [{}:{}] (#{}) {}",
        line,
        "^",
        error_ref.line,
        error_ref.column,
        error_ref.error_type.get_message().unwrap(),
        error_ref.error_type,
        width = error_ref.column as usize
    )
}
...
mod test {
    ...
    fn test_all_error_has_number() {
        for error_enum in super::KaramelErrorType::iter() {
            ...
            if error_enum != KaramelErrorType::GeneralError("".to_string()) && error_message.is_empty() {
                assert!(false, "'{:?}' hata mesaji yok", error_enum)
            }

            match error_enum.get_message() {
                Some(error_message) => assert!(!error_message.is_empty(), "'{:?}' sira numarasi bos olamaz", error_enum),
    ...
    fn test_error_message_generator() {
        ...
        assert!(!error_message.is_empty(), "Hata mesaji bos uretilemez");
