...
use crate::{
    buildin::{
        base_functions,
        class::{dict, get_empty_class, list, number, proxy, text},
        debug, io, Class, Module, ModuleCollection,
    },
...
use super::{
    function::{FunctionFlag, FunctionReference, FunctionType},
    module::OpcodeModule,
    KaramelPrimative, StaticStorage,
};
...
impl Default for KaramelCompilerContext {
    fn default() -> Self {
        Self::new()
    }
}
...
impl KaramelCompilerContext {
    pub fn new() -> KaramelCompilerContext {
        ...
        compiler
            .primative_classes
            .push(number::get_primative_class());
        ...
        compiler
            .primative_classes
            .push(proxy::get_primative_class());
        compiler.primative_classes.push(get_empty_class());
            ...
            for (_, module) in self.modules.iter() {
                for function_reference in module.get_methods().iter() {
                    let result = match &function_reference.callback {
                        FunctionType::Native(_) => function_reference.module.get_path() == module_path && &function_reference.name == name.borrow(),
                        FunctionType::Opcode => {
                            &function_reference.name == name.borrow()
                                ...
                                && (function_reference.defined_storage_index == search_storage
                                    || function_reference
                                        .flags
                                        .contains(FunctionFlag::MODULE_LEVEL))
                        }
                    };
    ...
    pub fn get_class(&self, value: &KaramelPrimative) -> Rc<dyn Class> {
        unsafe {
            self.primative_classes
                .get_unchecked(value.discriminant())
                .clone()
        }
    }

    pub fn find_class(&self, name: String, _module_path: &[String], _start_storage_index: usize) -> Option<Rc<dyn Class>> {
        let primative_search = self
            .primative_classes
            .iter()
            .find(|&item| item.get_class_name() == name);
