    ...
    fn build(&self, module: Rc<OpcodeModule>, ast: &KaramelAstType, _: &KaramelAstType, options: &mut KaramelCompilerContext, storage_index: usize) -> Result<(), KaramelErrorType> {
        ...
        match ast {
            ...
            KaramelAstType::Symbol(string) => {
                if let Some(reference) = module.get_method(&string[..]) {
                    options
                        .storages
                        .get_mut(storage_index)
                        .unwrap()
                        .add_constant(Rc::new(KaramelPrimative::Function(reference, None)));
                };
                ...
                if let Some(reference) = class_search {
                    options
                        .storages
                        .get_mut(storage_index)
                        .unwrap()
                        .add_constant(Rc::new(KaramelPrimative::Class(reference)));
                };
            ...
            KaramelAstType::Assignment { variable, operator: _, expression } => {
            ...
            KaramelAstType::AccessorFuncCall { source, indexer, assign_to_temp: _ } => {
                    ...
                    KaramelAstType::Symbol(function_name) => {
                        ...
                        if let Some(reference) = function_search {
                            ...
                            options
                                ...
                                .get_mut(storage_index)
                                .unwrap()
                                .add_constant(Rc::new(KaramelPrimative::Text(Rc::new(function_name.to_string()))));
                        }
                    }
            ...
            KaramelAstType::Loop { loop_type, body } => {
                match loop_type {
                    ...
                    LoopType::Simple(control) => self.build(module.clone(), control, ast, options, storage_index)?,
                    LoopType::Endless => {}
                };
            ...
            KaramelAstType::Primative(primative) => {
                options
                    ...
                    .get_mut(storage_index)
                    .unwrap()
                    .add_constant(Rc::clone(primative));
            }
            ...
            KaramelAstType::Dict(dict) => {
                for dict_item in dict {
                    options
                        ...
                        .get_mut(storage_index)
                        .unwrap()
                        .add_constant(dict_item.key.clone());
            ...
            KaramelAstType::IfStatement { condition, body, else_body, else_if } => {
            ...
            KaramelAstType::None => {
                options
                    ...
                    .get_mut(storage_index)
                    .unwrap()
                    .add_constant(Rc::new(KaramelPrimative::Empty));
            }
            _ => (),
        };
        Ok(())
