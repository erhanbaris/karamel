...
use super::module::{get_modules, OpcodeModule};
...
impl InterpreterCompiler {
    pub fn compile(&self, main_ast: Rc<KaramelAstType>, context: &mut KaramelCompilerContext) -> CompilerResult {
        ...
        self.generate_functions(main_module.clone(), &functions, context)?;

        /* Prepare jump code for main function */
        context
            .opcode_generator
            .set_current_location(main_location.clone());
    ...
    pub fn check_prohibited_names<T: Borrow<String>>(&self, variable: T) -> Result<(), KaramelErrorType> {
        ...
        if PRIMATIVE_CLASS_NAMES
            .lock()
            .unwrap()
            .contains(variable.borrow())
        {
            return Err(KaramelErrorType::ReservedName(variable.borrow().to_string()));
        }
    ...
    fn generate_functions(&self, module: Rc<OpcodeModule>, functions: &[Rc<FunctionReference>], context: &mut KaramelCompilerContext) -> CompilerResult {
        for function in functions.iter() {
            ...
            context
                .opcode_generator
                .create_function_definition(function.clone());
            self.generate_opcode(module.clone(), function.opcode_body.as_ref().unwrap(), function.opcode_body.as_ref().unwrap(), context, function.storage_index)?;
        }
        ...
        match read_module_or_script(path.to_str().unwrap(), context) {
            Ok(content) => {
                ...
                if let Err(error) = parser.parse() {
                    return Err(KaramelErrorType::from(error));
                };
    ...
    fn generate_list(&self, module: Rc<OpcodeModule>, list: &[Rc<KaramelAstType>], upper_ast: &KaramelAstType, context: &mut KaramelCompilerContext, storage_index: usize) -> CompilerResult {
    ...
    fn generate_dict(&self, module: Rc<OpcodeModule>, dict: &[Rc<KaramelDictItem>], upper_ast: &KaramelAstType, context: &mut KaramelCompilerContext, storage_index: usize) -> CompilerResult {
    ...
    fn generate_func_call_by_name(&self, name: &String, module_path: &Vec<String>, arguments: &[Rc<KaramelAstType>], assign_to_temp: bool, context: &mut KaramelCompilerContext, storage_index: usize) -> Result<bool, KaramelErrorType> {
        let function_search = context.get_function(name.to_string(), module_path, storage_index);

        if let Some(function_ref) = function_search {
            ...
            match search_location {
                Some(location) => {
                    context
                        .opcode_generator
                        .create_call(location, arguments.len() as u8, assign_to_temp);
                    return Ok(true);
                }
        ...
        if let Some(location) = context.storages[storage_index].get_variable_location(name) {
            context.opcode_generator.create_load(location);
            context
                .opcode_generator
                .create_call_stack(arguments.len() as u8, assign_to_temp);
            return Ok(true);
        };
            ...
            match &**func_name_expression {
                KaramelAstType::Symbol(function_name) => {
                    ...
                    match search_location {
                        ...
                        _ => Err(KaramelErrorType::FunctionNotFound(function_name.to_string())),
                    }
                }

                KaramelAstType::FuncCall { func_name_expression, arguments, assign_to_temp: _ } => self.generate_func_call(module.clone(), func_name_expression, arguments, true, upper_ast, context, storage_index),
                _ => Err(KaramelErrorType::FunctionNotFound("<Bilinmeyen>".to_string())),
            }
        } else {
            Err(KaramelErrorType::FunctionNotFound("<Bilinmeyen>".to_string()))
        }
    }

    #[allow(clippy::too_many_arguments)]
    fn generate_func_call(&self, module: Rc<OpcodeModule>, func_name_expression: &KaramelAstType, arguments: &[Rc<KaramelAstType>], assign_to_temp: bool, upper_ast: &KaramelAstType, context: &mut KaramelCompilerContext, storage_index: usize) -> CompilerResult {
        /* Build arguments */
        for argument in arguments {
            self.generate_opcode(module.clone(), argument, upper_ast, context, storage_index)?;
        }
        ...
        match &func_name_expression {
            KaramelAstType::Symbol(function_name) => {
                ...
                match result {
                    ...
                    false => {
                        ...
                        Err(KaramelErrorType::FunctionNotFound(function_name.to_string()))
                    }
                }
            }
            ...
            KaramelAstType::ModulePath(names) => {
                ...
                match result {
                    true => Ok(()),
                    false => Err(KaramelErrorType::FunctionNotFound(names[names.len() - 1].to_string())),
                }
            }
            _ => {
    ...
    fn generate_break(&self, _: &KaramelAstType, context: &mut KaramelCompilerContext, _: usize) -> CompilerResult {
        ...
        context
            .opcode_generator
            .add_break_location(location.clone());
    ...
    fn generate_continue(&self, _: &KaramelAstType, context: &mut KaramelCompilerContext, _: usize) -> CompilerResult {
        ...
        context
            .opcode_generator
            .add_continue_location(location.clone());
        context.opcode_generator.create_jump(location.clone());
        Ok(())
    }
    ...
    fn generate_loop(&self, module: Rc<OpcodeModule>, loop_type: &LoopType, body: &KaramelAstType, upper_ast: &KaramelAstType, context: &mut KaramelCompilerContext, storage_index: usize) -> CompilerResult {
        ...
        let (variable, control, increment) = match loop_type {
            LoopType::Endless => (None, None, None),

            LoopType::Simple(control) => (None, Some(control.clone()), None),

            LoopType::Scalar { variable, control, increment } => (Some(variable.clone()), Some(control.clone()), Some(increment.clone())),
        };
        ...
        if let Some(compare_location) = &compare_location {
            context
                .opcode_generator
                .subtract_location(compare_location.clone(), end_location.clone(), compare_location.clone());
        }

        context
            .opcode_generator
            .set_breaks_locations(end_location.clone());
        context
            .opcode_generator
            .set_continues_locations(start_location.clone());
    ...
    fn generate_symbol(&self, module: Rc<OpcodeModule>, variable: &String, _: &KaramelAstType, context: &mut KaramelCompilerContext, storage_index: usize) -> CompilerResult {
        ...
        match storage.get_variable_location(variable) {
            ...
            None => Err(KaramelErrorType::ValueNotFoundInStorage),
        }
    }
        ...
        match variable {
            KaramelAstType::Symbol(symbol) => {
                ...
                if let KaramelAstType::Primative(primative) = expression_ast {
                    if mem::discriminant(&**primative) != mem::discriminant(&KaramelPrimative::List(RefCell::new(Vec::new()))) && *operator == KaramelOperatorType::Assign {
                        ...
                        context
                            .opcode_generator
                            .create_fast_store(primative_location, location);
                        return Ok(());
                    }
                }
    ...
    fn generate_prefix_unary(&self, module: Rc<OpcodeModule>, operator: &KaramelOperatorType, expression: &KaramelAstType, assign_to_temp: &Cell<bool>, _: &KaramelAstType, context: &mut KaramelCompilerContext, storage_index: usize) -> CompilerResult {
        ...
        if let KaramelAstType::Symbol(variable) = expression {
            let location = match context
                ...
                .get_mut(storage_index)
                .unwrap()
                .get_variable_location(variable)
            {
    ...
    fn generate_if_condition(
        ...
        */
        ...
        for else_if_item in else_if {
            ...
            context
                .opcode_generator
                .subtract_location(if_failed_location.clone(), context.opcode_generator.build_current_location(), if_failed_location.clone());
