...
use crate::{
    compiler::{
        function::{FunctionFlag, IndexerGetCall, IndexerSetCall},
        GetType,
    },
    types::VmObject,
};
...
use crate::compiler::{
    function::{FunctionReference, NativeCall},
    KaramelPrimative,
};
...
impl ModuleCollection {
    pub fn new() -> ModuleCollection {
        ModuleCollection { modules: HashMap::new() }
    }
    ...
    pub fn iter(&self) -> ModuleCollectionIterator {
        ModuleCollectionIterator { iter: self.modules.iter().clone() }
    }
    ...
    pub fn has_module(&self, module_path: &Vec<String>) -> bool {
        self.modules
            .iter()
            .find_map(|(key, module)| {
                if module.get_path() == module_path {
                    Some(key)
                } else {
                    None
                }
...
impl Module for DummyModule {
    ...
    fn get_method(&self, _: &str) -> Option<Rc<FunctionReference>> {
        None
    }
