...
use crate::{arc_bool, arc_empty, expected_parameter_type, n_parameter_expected};
use crate::{
    buildin::Class,
    compiler::function::{FunctionParameter, NativeCallResult},
};
...
mod tests {
    ...
    fn test_add_3() {
        ...
        let parameter = FunctionParameter::new(&stack, Some(obj), stack.len(), stack.len() as u8, &stdout, &stderr);
    ...
    fn test_insert_1() {
        ...
        let result = add(FunctionParameter::new([arc_text!("dünya")].as_ref(), Some(obj), 1_usize, 1_u8, &stdout, &stderr));
        assert!(result.is_ok());
        ...
        let result = insert(FunctionParameter::new([arc_number!(0), arc_text!("merhaba")].as_ref(), Some(obj), 2_usize, 2_u8, &stdout, &stderr));
        assert!(result.is_ok());

        match &*list {
            KaramelPrimative::List(l) => {
                assert_eq!(l.borrow().len(), 2);
                assert_eq!(l.borrow().first().unwrap().deref(), Rc::new(primative_text!("merhaba")));
